...
system.time(x <- fromJSON("~/tmp/Historiquedespositions.json"))
```
...
Plotting on map
```{r}
germany <- get_map(location = 'France', zoom = 6)
...
Close to Versailles
```{r}
munster <- get_map(location = 'Viroflay', zoom = 10)
...
Velocity of data points
```{r}
loc_2 <- loc[which(!is.na(loc$velocity)), ]

munster <- get_map(location = 'Versailles', zoom = 10)
...
loc3 <- subset(loc, loc$time > as.POSIXct('2016-01-01 0:00:01'))
...
loc3$time.p1 <- shift.vec(loc3$time, -1)
loc3$time.to.prev <- as.numeric(loc3$time - loc3$time.p1, units = "secs")
...
loc4 <- subset(loc3, loc3$time.to.prev > 0)
loc4$speed <- loc4$dist.to.prev / loc4$time.to.prev

loc3 <- subset(loc4, loc4$speed < 50)
```
Distance in km
...
```{r}
ggplot(subset(activities_2, main_activity != "still"), aes(x = main_activity, group = main_activity, fill = main_activity)) + 
  geom_bar()  + 
  guides(fill = FALSE) +
  my_theme() +
  labs(
