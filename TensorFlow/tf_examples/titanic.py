...
print("===> Linear accuracy", accuracy_score(tflr.predict(X_test), y_test))
...
print("===> DNN accuracy", accuracy_score(classifier.predict(X_test), y_test))
...
classifier = skflow.TensorFlowEstimator(model_fn=dnn_tanh,
    n_classes=2, batch_size=128, steps=500, learning_rate=0.05)
classifier.fit(X_train, y_train)
print("===> DNN tanh accuracy", accuracy_score(classifier.predict(X_test), y_test))
